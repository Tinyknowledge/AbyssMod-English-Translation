# 鸡渊 (Abyss) — English Translation Mod [FANZA Version]
A Small Note

This is my first time working on a mod/project like this, so please keep in mind that it may not be perfect. I originally started this for myself and decided to share it in case other English-speaking players find it useful.

I’ve put a lot of time into translating the game and fixing UI issues, but there may still be mistranslations, untranslated text, bugs, or things I simply missed.

I’ll try to keep the mod updated when I have the time, especially when new characters, events, or content are released. However, I can’t promise that every update will be immediate—or that I’ll always be able to maintain it.

This is just a personal project I made for fun, so please don’t judge it too harshly. 

If you find an issue, feel free to report it. Just be patient with me—I’m learning as I go!

Most importantly, I hope the mod makes the game a little easier and more enjoyable for English-speaking players. 

# Credits & Note

This English translation is made for [AbyssMod](https://github.com/anosu/AbyssMod), created by anosu.

I originally made this translation for myself so I could enjoy the game in English. I'm sharing it here in case anyone else wants to use it too.

This is an unofficial personal fan translation. All original game content belongs to its respective owners.
